...
$reports->execute(array($_SESSION['report_id']));
...
if(!empty($_POST)){
	...
	// DBへの登録について
	$statement = $db -> prepare('UPDATE reports SET member_id=?, member_code=?, member_name=?, machine_code=?, machine_name=?, parts_code=?, parts_name=?, production_num=?, production_date=?, start_time=?, finish_time=?, lunch_time=?, breakdown_time1=?, breakdown_reason1=?, comment1=?, breakdown_time2=?, breakdown_reason2=?, comment2=?, breakdown_time3=?,  breakdown_reason3=?, comment3=?, modified=NOW() WHERE report_id=?');
	// NOW()は登録された日時
...
<div id="wrap">
  ...
  <div id="content">
    <p>編集した内容を確認して、「実績編集する」ボタンをクリックしてください</p>
    <h4>実績</h4>
    <form action="" method="post">
    	<input type="hidden" name="action" value="submit" />  
        <table>
          <tr>
          <td>レポートNo.　　：<?php print(htmlspecialchars($_SESSION['report_id'], ENT_QUOTES)); ?>
          </td>
          </tr>
          <tr>
